...
import blackJack.calculation.{Rational, probabilityStatistics}
...
object ProbabilityStatistics {
  ...
  戻値          重複分の値を加算した確率統計
  */
  def apply[I <: Seq[(K,Rational)],K](elems: I): ProbabilityStatistics[K] = {
    new ProbabilityStatistics().addValue(elems)
  }
...
*/
final class ProbabilityStatistics[K] private (elems: Map[K,Rational]) extends immutable.Map[K,Rational] {
  ...
  */
  override def removed(key: K): ProbabilityStatistics[K] =
    ProbabilityStatistics(elems.removed(key))
  ...
  */
  def addValue(key: K, value: Rational): ProbabilityStatistics[K] =
    key match {
      case k if this.contains(k) => ProbabilityStatistics(this.updated(key, value + this(k)))
      case _ => ProbabilityStatistics(this.updated(key,value))
    }
  ...
  戻値        ProbabilityStatistics     追加した確率統計
  */
  def addValue(elem: (K,Rational)): ProbabilityStatistics[K] = addValue(elem._1,elem._2)
  ...
  戻値        ProbabilityStatistics       追加した確率統計
  */
  def addValue(elems: Seq[(K,Rational)]): ProbabilityStatistics[K] = elems.foldLeft(this)((acc,x) => acc.addValue(x))
}
