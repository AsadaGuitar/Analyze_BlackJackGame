...
class BasicStrategy(val userHand: Hand, val dealerHand: Hand) extends Strategy {
  ...
  override def nextAction() = userHand match {
    case hand if hand.isExist(1) =>
      val userScore = (hand -1).sum
      val dealerScore = dealerHand.sum
      softHand(userScore, dealerScore)
    case _ =>
      val userScore = userHand.sum
      val dealerScore = dealerHand.sum
